...
export function Hero() {
  ...
  // const { data, error } = useSWR<Listing>(`${marketplaceServerUrl()}/listings/last`, fetcher)
  ...
  return (
    <section
      ...
      <div className='relative hover:shadow-lg m-4 rounded-3xl md:max-w-[340px] xl:max-w-[256px]' style={{ minWidth: "256px" }}>
        {/* <NavLink to={`/t/${data?.mint_pubkey}`} className='rounded-3xl'> */}
        <NavLink to={`/t/`} className='rounded-3xl'>
          {/* <Image
            src={tokenImage(data?.token!)!}
            size='2xl'
            placeholder="blur"
