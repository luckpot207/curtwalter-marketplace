      ...
      <div className="group relative h-full flex flex-col justify-between">
        <div className="w-full bg-gray-200 rounded-2xl overflow-hidden aspect-w-1 aspect-h-1">
          <Image
            ...
            size='xl'
            className={`w-full h-full object-center object-cover group-hover:opacity-75 ${props.sold ? "opacity-50" : ""
              }`}
          />
        </div>
        <div className="mt-2">
          ...
          <div
            className={`bg-white dark:bg-zinc-900 items-number-box-shadow text-sm w-full font-semibold rounded-2xl py-2 flex px-3 ${props.sold ? "justify-center" : "justify-between"
              }`}
          >
            {props.sold ? (
              "Sold"
