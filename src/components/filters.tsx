...
const test_filterOpenedSections = [
  {
...
const test_trait = {
  key: "1",
  type: "status",
  values: ["favorite", "cool", "gold", "trust"],
  numbers: [{
    value: "gold",
    amount: 2312,
    floor: 1000
...
function FilterItem(props: {
  ...
  const { section } = props;
  // const filters = useSelector((data:any) => data.filters);
  // const openedSections = useSelector((data:any) => data.filterOpenedSections);
  const filters = test_filters;
  const openedSections = test_filterOpenedSections;
...
export function MobileFilters(props: {
  ...
  //   (data: any) => data.collectionMetas[props.idOrSlug]
  // );
  for (let i = 0; i < 12; i++) {
    test_trait.key = String(i);
    test_CollectionMeta.traits.push(test_trait);
  }
        ...
        >
          <div className="ml-auto relative max-w-xs w-full h-full bg-white dark:bg-zinc-800 shadow-xl pt-4  flex flex-col">
            ...
            <div className="flex flex-col flex-1 overflow-y-scroll h-full">
              ...
              {test_CollectionMeta?.traits
                ?.sort((a: any, b: any) => a.key?.localeCompare(b.key ?? "") ?? 0)
                .map((section: any) => (
...
export function Filters(props: { idOrSlug: string }) {
  ...
  return (
    <form className="hidden h-screen overflow-y-scroll lg:block lg:w-60 xl:w-80 mr-4 sticky top-16">
      ...
      {test_CollectionMeta.traits
        ?.sort(
          (a: any, b: any) => (a.key ?? "unknown").localeCompare(b.key ?? "unknown") ?? 0
        )
        .map((section: any) => (
          <FilterItem key={section.key} section={section} />
        ))}
