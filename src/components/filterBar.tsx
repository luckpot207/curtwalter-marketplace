...
export default function FilterBar(props: {
  ...
  // const { orderBy, filters, refreshing } = useSelector((data:any) => ({
  //   filters: data.filters,
  //   orderBy: data.orderBy,
  //   refreshing: data.filterIsInProgress,
  // }));
  const test_filters = [{}];
  const test_orderBy = undefined;
  const test_refreshing = false;
  ...
  return (
    <div className="bg-gray-200 dark:bg-zinc-800 flex items-center mb-4 px-4 py-4 z-10 w-full rounded-2xl">
      ...
      <div className="flex-1 sm:items-center sm:px-1 lg:px-1 hidden lg:flex">
        ...
        <div className="mt-2 sm:mt-0 sm:ml-4">
          <div className="-m-1 flex flex-wrap items-center">
            {test_filters.map((activeFilter:any) => (
              <FilterChip
                filter={activeFilter}
                key={
                  activeFilter.type +
            ...
            {test_filters.length > 0 && <RemoveFiltersButton />}
          </div>
        </div>
      </div>
      ...
      >
        ...
        <FiFilter className="w-6 h-6" aria-hidden="true" />
        ...
        <div className="w-full">
          <Menu.Button className="group inline-flex text-sm font-medium text-black dark:text-gray-300 dark:hover:bg-gray-500 pl-4 py-2 w-full justify-between">
            <p className="text-sm font-medium text-black dark:text-white hover:text-gray-900">
              {OrderTitles["HIGHEST_CURRENT_OFFER"] ? OrderTitles["HIGHEST_CURRENT_OFFER"] : "Sortation"}
            </p>
            <FiChevronDown
                ...
                <Menu.Item key={option}>
                  {({ active }) => (
                    <button
                      ...
                      onClick={() => {}}
                      disabled={option === test_orderBy}
                      className={classNames(
                        option === test_orderBy ? "font-medium " : "",
                        active ? "bg-gray-100" : "",
