...
const filterHistory = (h: TradingHistoryItem) => {
...
const test_TradingHistoryItems = [{
  signature: "signature",
  mintPubkey: "fef98u8832ujf2dwj9832f",
  user: "john",
  userName: "luck007",
  tradingType: "SALE",
  createdAt: "1678393314524",
  price: 2342342,
  toPubkey: "feajfi90j0909m",
  toName: "Ted",
  marketplace: "alpha.art",
  escrowAccount: "fefei",
  program: "fjiejoafij",
  candyMachineId: "ajefjaje",
  serviceFee: 100,
  image: MoonkeesNft,
  name: "oldTrading",
}, {
  signature: "signature",
  mintPubkey: "fef98u8832ujf2j9832f",
  user: "john",
  userName: "luck007",
  tradingType: "SALE",
  createdAt: "1678393314524",
  ...
  signature: "signature",
  mintPubkey: "fef98u883f2ujf2j9832f",
  user: "john",
  userName: "luck007",
  tradingType: "SALE",
  createdAt: "1678393314524",
...
export function TradingHistory(props: {
  ...
  const loadData = (fetchMore: boolean) => {
    ...
    // }).then(async (res) => {
      // const results = test_TradingHistoryItems.filter(filterHistory);
      const results = test_TradingHistoryItems;
      if (test_TradingHistoryItems.length === 0) {
        setHasNoNext(true);
        if (fetchMore) {
      ...
      lastCreatedAt = test_TradingHistoryItems[test_TradingHistoryItems.length - 1].createdAt;

      if (results.length === 0) {
        // Fix this : results being 0 doesn't guarantee that there is no next page
        if (test_TradingHistoryItems.length < PAGE_SIZE) {
          setHasNoNext(true);
          if (fetchMore) {
            fetchingNextPage = false;
          } else {
      ...
      if (fetchMore) {
        setHistory((history:any) => [...history, ...results]);
        fetchingNextPage = false;
      } else {
        setHistory(results as TradingHistoryItem[]);
        setLoading(false);
      }
    // });
  };
        ...
        <div className="flex p-4 border-b border-gray-300 dark:border-zinc-500 items-center">
          <div className="flex flex-1 flex-col">
            <div className="flex flex-1 items-center">
              <FiTrendingUp className="flex-shrink-0 mr-1 h-5 w-5" />
          ...
          >
            ...
            {!loading && !history.length && (
              <div className="flex flex-col items-center justify-center h-44">
                <FiTrendingUp className="flex-shrink-0 mr-1 h-9 w-9 mb-2" />
                    ...
                    <div className="p-4 flex-1 w-12">
                      <div className="flex flex-row items-center">
                        {historyItem.tradingType === "SALE" && (
                          <FiShoppingCart className="flex-shrink-0 mr-3 h-5 w-5" />
                        )}
                        {historyItem.tradingType === "LISTING" && (
                          <FiTag className="flex-shrink-0 mr-3 h-5 w-5" />
                        )}
                        {historyItem.tradingType === "OFFER" && (
                          <HiHand className="flex-shrink-0 mr-3 h-5 w-5" />
                        )}
                        {historyItem.tradingType === "UNLIST" && (
                          <FiMinusCircle className="flex-shrink-0 mr-3 h-5 w-5" />
                        )}
                        <span>
                          {historyItem.tradingType === "UNLIST"
                            ? "DELIST"
