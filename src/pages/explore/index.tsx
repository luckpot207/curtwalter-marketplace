...
export function Explore() {
    ...
    const cateDatas: CategoryData[] = [];
    for (let index = 0; index < 8; index++) {
        CategoryData_.id = String(index);
        cateDatas.push(CategoryData_);
    }
    const collDatas: BaseCollectionData[] = [];
    for (let index = 0; index < 12; index++) {
        CollectionData_.id = String(index);
        collDatas.push(CollectionData_);
    }
    ...
    const [categoryFilter, setCategoryFilter] =
        useState<string[]>(catFilter);

    const [collectionList, setCollectionList] = useState<
        BaseCollectionData[]
    >([]);
    const [collectionListSort, setCollectionListSort] = useState<{
        order: string;
        dir: string;
    }>({ order: "title", dir: "asc" });
    ...
    useEffect(() => {
        setCategoryList(cateDatas);
    }, []);

    useEffect(() => {
        setCollectionListNextPage(false);

        setCollectionList(collDatas);
        setCollectionListOffset(collDatas.length);
        setCollectionListNextPage(collDatas.length > 0);
    ...
    const fetchNextPage = () => {
        ...
        if (collDatas.length === 0) {
            ...
            const array = [...collectionList, ...collDatas];
            const newArray: any[] = [];
            ...
            setCollectionListOffset(collectionListOffset + collDatas.length);
                ...
                <div className="w-full flex justify-center gap-4 my-8 flex-wrap text-left">
                    {categoryList.map((category:any, k:number) => (
                        <Button size="md"
                            ...
                            color={
                                theme === 'light'
                                    ? categoryFilter.includes(category.id.toString()) ? 'dark' : 'light'
                                    : categoryFilter.includes(category.id.toString()) ? 'light' : 'dark'
                            }
                    ...
                    <div className="flex justify-between w-full px-4 sm:px-6">
                        <div className="relative w-1/2 sm:w-1/3 md:w-1/4 mr-1">
                            ...
                            <input
                                ...
                                placeholder={`${windowSize.width && windowSize.width > 480
                                    ? "Search Collections"
                                    : ""
                                    }`}
                                autoComplete="off"
